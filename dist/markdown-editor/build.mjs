...
const result = await build({entryPoints: ['editor.js'], outdir, entryNames: 'live-editor', bundle: true,
    ...
    plugins: [{name: 'reuse-katex-css', setup(build) {
        build.onLoad({filter: /katex[\\/]dist[\\/]katex\.min\.css$/}, () => ({contents: '', loader: 'css'}));
    }}]});
...
await writeFile(path.join(outdir, 'licenses', 'milkdown-editor.txt'), notices.join('\n\n' + '='.repeat(72) + '\n\n'));
