...
Vertical Tabs 的 logo、title、cwd 和 working ring 严格消费 focused Surface；同 Tab 的其他 pane 只能贡献 trailing `needsAttention`/`error`/`done`。Agent manifests 提供 optical scale，使品牌 glyph 与 terminal/cloud 图标保持一致视觉尺寸。Agent 启动时 OpenAI/Claude/Pi template glyph 替换 terminal/cloud；idle 不显示 ring，working 使用旋转的四分之一圆 progress indicator；等待审批、完成和失败继续使用右侧 status slot。用户选中已完成 Tab 时，`done` acknowledge 为 `idle`，提醒立即消失但 agent identity 保留。Agent context 结束后恢复 canonical terminal/cloud icon。Horizontal Tabs 保持 Ghostty 原生 presentation，不注入自定义状态 UI。Hook 只能改变展示状态，不获得 Terminal Control、文件系统或网络能力。
