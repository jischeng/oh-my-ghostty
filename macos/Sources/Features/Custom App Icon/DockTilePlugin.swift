...
class DockTilePlugin: NSObject, NSDockTilePlugIn {
    // WARNING: An instance of this class is alive as long as OMG's icon is
    ...
    #if DEBUG
    private let omgUserDefaults = UserDefaults(suiteName: "com.jischeng.omg.debug")
    #else
    private let omgUserDefaults = UserDefaults(suiteName: "com.jischeng.omg")
    #endif
    ...
    func setDockTile(_ dockTile: NSDockTile?) {
        // If no dock tile or no access to OMG defaults, we can't do anything.
        guard let dockTile, let omgUserDefaults else {
            iconChangeObserver = nil
            return
        }

        // Try to restore the previous icon on launch.
        iconDidChange(omgUserDefaults.appIcon, dockTile: dockTile)

        // Setup a new observer for when the icon changes so we can update. This message
        // is sent by the primary OMG app.
        iconChangeObserver = DistributedNotificationCenter
            .default()
            .publisher(for: .ghosttyIconDidChange)
            .map { [weak self] _ in self?.omgUserDefaults?.appIcon }
    ...
    private func resetIcon(dockTile: NSDockTile) {
        ...
        if #available(macOS 26.0, *) {
            // Reset to the compiled OMG app icon.
            appIcon = nil
        } else {
