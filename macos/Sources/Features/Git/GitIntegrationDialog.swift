        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let commit {
                ...
                if commit.parentIDs.count > 1 {
                    Picker(GitL10n.text("Mainline parent"), selection: $mainline) {
                ...
                Picker(GitL10n.text(kind == .rebase ? "Base branch" : "Source branch"), selection: $source) {
                    Text(GitL10n.text("Select branch…")).tag("")
                    ForEach(kind == .review ? locals : branches, id: \.id) { branch in Text(branch.name).tag(branch.id) }
                }
            }
            Picker(GitL10n.text(kind == .rebase ? "Branch to rebase" : "Target branch"), selection: $target) {
                Text(GitL10n.text("Select branch…")).tag("")
            ...
            if kind == .rebase {
                ...
                HStack {
                    ...
                    Button(GitL10n.text("Generate")) { generate() }.disabled(!valid || busy)
                }
            ...
            Text(GitL10n.text(kind == .review
                ? "Creates a remote PR/MR using local gh/glab. Both branches must already match origin. Nothing is pushed automatically."
