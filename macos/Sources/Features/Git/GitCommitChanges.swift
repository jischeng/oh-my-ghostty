    ...
    init(data: Data) throws {
        var fields = try data.split(separator: 0).map { field in
            guard let text = String(bytes: field, encoding: .utf8) else {
                throw GitDiffServiceError.gitFailed("Git returned a path that is not UTF-8.")
            }
            return text
        }[...]
        ...
        while fields.first?.hasPrefix(":") == true {
            ...
            guard record.count == 5, let status = record.last, let path = fields.popFirst() else {
                throw GitDiffServiceError.gitFailed("Git returned an incomplete changed-file record.")
            }
        ...
        while let field = fields.popFirst() {
            ...
            guard values.count == 3 else {
                throw GitDiffServiceError.gitFailed("Git returned incomplete change statistics.")
            }
            let path: String
            if values[2].isEmpty {
                guard fields.popFirst() != nil, let newPath = fields.popFirst() else {
                    throw GitDiffServiceError.gitFailed("Git returned incomplete rename statistics.")
                }
                path = newPath
            } else { path = String(values[2]) }
            guard paths.insert(path).inserted else {
                throw GitDiffServiceError.gitFailed("Git returned duplicate change statistics.")
            }
            if values[0] == "-" && values[1] == "-" {
                ...
                guard added <= Int.max - additions, removed <= Int.max - deletions else {
                    throw GitDiffServiceError.gitFailed("Git change statistics exceeded the supported range.")
                }
                additions += added
                deletions += removed
            } else {
                throw GitDiffServiceError.gitFailed("Git returned invalid change statistics.")
            }
        }
        guard paths == Set(files.map(\.path)) else {
            throw GitDiffServiceError.gitFailed("Git returned mismatched files and change statistics.")
        }
