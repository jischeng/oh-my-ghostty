...
- Core-owned `InspectorRegistry` / `RightInspectorHost`；Horizontal/Vertical 共用 `NSWindowTabGroup` layout state，`InspectorPresentationStore` 持久化 UI state，右上 titlebar control / `⌘⇧I` 切换，pane switch strip 预留 Git/SSH 等扩展；Plugin Inspector 仅贡献经过 owner validation 的 typed content，`builtin.files` 通过同一数据边界异步提供最小文件列表。
